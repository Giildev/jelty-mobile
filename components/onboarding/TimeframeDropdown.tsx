...
interface TimeframeDropdownProps {
  label: string;
  value: GoalTimeframe | null;
  onChange: (timeframe: GoalTimeframe) => void;
  error?: string;
  placeholder?: string;
}

const timeframes: GoalTimeframe[] = [
  "4_weeks",
  "8_weeks",
  "12_weeks",
  "16_weeks",
...
export function TimeframeDropdown({
  ...
  const handleSelect = (timeframe: GoalTimeframe) => {
    onChange(timeframe);
    setIsOpen(false);
  };
    ...
    <View className="mb-4">
      ...
      >
        <Text
          ...
          numberOfLines={1}
        >
          {value ? timeframeLabels[value] : placeholder}
        </Text>
        <Ionicons
